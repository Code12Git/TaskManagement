...
import { ADD_TASK_FAILURE, ADD_TASK_REQUEST, ADD_TASK_SUCCESS, DELETE_TASK_FAILURE, DELETE_TASK_REQUEST, GET_TASK_FAILURE, GET_TASK_REQUEST, GET_TASK_SUCCESS, GET_TASKS_FAILURE, GET_TASKS_REQUEST, GET_TASKS_SUCCESS, UPDATE_TASK_FAILURE, UPDATE_TASK_REQUEST, UPDATE_TASK_SUCCESS } from "../actionTypes/actionTypes"
...
export const deleteOne = (id:string) => async (dispatch: Dispatch) =>  {
    ...
    try{
        ...
        dispatch({ type: ADD_TASK_SUCCESS, payload: id });
    }catch(err){
        const error = err as { response: { data: { code: { message: string } } } };
...
export const getAll = () => async (dispatch:Dispatch) => {
    ...
    try{
        const getAllTask = await publicRequest.get('/tasks')
        dispatch({ type:GET_TASKS_SUCCESS,payload:getAllTask })
    }catch(err){
        const error = err as {response : {data: {code:{message:string}}}}
        dispatch({ type:GET_TASKS_FAILURE,payload:error.response.data.code.message || "Something went wrong" })
    }
}